...
E3 MIDI{STATUS==cc}{CC_FUNCTION==70}("{}"->f"{round(CC_VALUE_SCALED(30, 110))}") [BLOCK|DEBOUNCE]->
{
    pid=$(focused-pid.sh)
    mc-cli --pid $pid send chat-local "$(mc-cli --pid $pid set-fov {})"
}
E3 MIDI{STATUS==cc}{CC_FUNCTION==75}("{}"->f"{CC_VALUE_SCALED(0, 1):.2f}") [BLOCK|DEBOUNCE]->
{
    pid=$(focused-pid.sh)
