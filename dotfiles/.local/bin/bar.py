...
async def watch_i3_forever(task_group, bar_event_queue, workspace_switch_queue):
    ...
    async def switch_workspaces_forever():
        ...
        # and manually focus the last focused container on the corresponding
        ...
        while True:
            ...
            try:
                while True:
                    workspace_id = workspace_switch_queue.get_nowait()
            except asyncio.QueueEmpty:
                ...
            get_tree_future = asyncio.get_running_loop().create_future()
            await send_message(writer, MessageType.GET_TREE)
            node = await get_tree_future
            try:
                node = next(
                    workspace
                    for workspace in get_workspace_nodes(node)
                    if workspace["id"] == workspace_id
                )
            except StopIteration:
                continue
            while node["focus"]:
                last_focused_child_id = node["focus"][0]
                node = next(
                    node
    ...
    while True:
        ...
        match payload_type:
            ...
            case MessageType.GET_TREE:
                get_tree_future.set_result(response)
...
async def update_bar_forever(task_group, bar_event_queue, workspace_switch_queue):
    ...
    DARK_BACKGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"48;2;66;123;88"
    GRAY_BACKGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"48;2;40;40;40"
    LIGHT_GRAY_BACKGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"48;2;80;73;69"
    WHITE_BACKGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"48;2;40;40;40"
    BLACK_FOREGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"38;2;235;219;178"
    DARK_GRAY_FOREGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"38;2;242;229;188"
    GRAY_FOREGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"38;2;146;131;116"
    LIGHT_GRAY_FOREGROUND = CHARACTER_ATTRIBUTES_TEMPLATE % b"38;2;251;241;199"
        ...
        while True:
            ...
            try:
                mouse_event_region = next(
                    ...
                    for element_region, (
                        start_column,
                        end_column,
                    ) in mouseable_bar_element_regions.items()
            ...
            match mouse_event_region:
                ...
                case MouseEventRegion.WORKSPACES:
                    ...
                    workspace_switch_queue.put_nowait(workspaces[label_index]["id"])
